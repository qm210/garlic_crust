...
/*
macro_rules! process_loop {
    ($op:tt, $action:expr, $sequence:expr, $offset:expr) => {
        let mut output = [0.; BLOCK_SIZE];

        let mut next_event = if $op.seq_cursor == $sequence.len() { None } else {
            Some(&$sequence[$op.seq_cursor])
        };

        for sample in 0 .. BLOCK_SIZE {
            let time: TimeFloat = (sample + $offset) as TimeFloat / SAMPLERATE;

            if next_event.is_some() {
                let event = next_event.unwrap();
                while $op.seq_cursor < $sequence.len() && event.time <= time {
                    match &event.message {
                        SeqMsg::NoteOn => {
                            $op.playhead = 0.;
                        },
                    ...
                    if $op.seq_cursor == $sequence.len() {
                        next_event = None;
                        break;
                    } else {
                        next_event = Some(&$sequence[$op.seq_cursor]);
                    }
                }
            }
...
pub fn envA_build(operator: &mut Envelope, sequence: &[SeqEvent], block_offset: usize) -> Edge {
    ...
    Edge::Array(output)
    */
    //process_loop!(operator, 0.1339, sequence, block_offset)
